# Copyright Notice

Copyright © 2026 KAVASRI Technologies.

All rights reserved.

ClearByte, including its software, source code, binaries, release packages, documentation, user interface designs, graphics, branding elements, logos, icons, screenshots, and associated materials are protected by applicable copyright and intellectual property laws.

The contents of this repository are provided solely for distribution of official ClearByte release packages and related documentation.

No permission is granted to:

* Copy or reproduce proprietary software components
* Modify or create derivative works
* Reverse engineer released binaries
* Redistribute modified versions
* Repackage application releases
* Use ClearByte trademarks, branding, or visual assets without authorization

Unauthorized use, reproduction, modification, redistribution, or commercial exploitation may result in legal action under applicable intellectual property laws.

For licensing, partnership, business, or legal inquiries:

Email: [inquiry.contactus5@gmail.com](mailto:inquiry.contactus5@gmail.com)

Published by: KAVASRI Technologies

Application: ClearByte

Package Name: com.clearbyte.app
